...
exports.achievementsRouter.get('/', (req, res) => {
    try {
        const achievements = init_1.db.prepare(`
      SELECT 
        a.id,
        a.name,
        a.description,
...
exports.achievementsRouter.post('/check', (req, res) => {
    try {
        const newAchievements = [];
        // Get current user stats
        const stats = getUserStats();
        // Check each achievement
        const unearned = init_1.db.prepare(`
      SELECT a.*
      FROM achievements a
      LEFT JOIN user_achievements ua ON a.id = ua.achievement_id AND ua.user_id = 1
      WHERE ua.id IS NULL
...
function getUserStats() {
    // Total activities
    const totalActivities = init_1.db.prepare(`
  ...
  `).get();
    ...
    const daysActive = init_1.db.prepare(`
    SELECT COUNT(DISTINCT date) as days 
    FROM activities 
    WHERE user_id = 1
  `).get();
    // Current streak (consecutive days with activities)
    const recentDates = init_1.db.prepare(`
