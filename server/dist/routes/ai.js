...
exports.aiRouter.post('/analyze-footprint', auth_1.authenticateToken, async (req, res) => {
    try {
      ...
      LIMIT 20
    `).all(userId);
        const totalCarbon = activities.reduce((sum, activity) => sum + activity.carbon_footprint, 0);
        const prompt = `
    ...
    ${activities.map((a) => `- ${a.category_name}: ${a.description} (${a.carbon_footprint}kg CO₂) on ${a.date}`).join('\n')}
...
exports.aiRouter.post('/suggest-activities', auth_1.authenticateToken, async (req, res) => {
    try {
...
exports.aiRouter.post('/calculate-carbon', auth_1.authenticateToken, async (req, res) => {
    try {
...
// AI-powered eco tips
exports.aiRouter.get('/daily-tip', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.userId;
        // Get user's most common category
        const categoryStats = init_1.db.prepare(`
      SELECT 
        ac.name,
        COUNT(*) as count,
...
exports.aiRouter.post('/suggest-goals', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.userId;
        // Get user's carbon footprint data
        const stats = init_1.db.prepare(`
      SELECT 
